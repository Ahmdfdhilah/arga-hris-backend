...
class AttendanceQueries:
    ...
    async def get_by_employee_and_date(self, employee_id: int, attendance_date: date) -> Optional[Attendance]:
        result = await self.db.execute(
            select(Attendance).where(
                and_(Attendance.employee_id == employee_id, Attendance.attendance_date == attendance_date)
            )
        )
        return result.scalar_one_or_none()
    ...
    async def count_by_employees(
        ...
        query = select(func.count(Attendance.id)).where(Attendance.employee_id.in_(employee_ids))
        if start_date:
            query = query.where(Attendance.attendance_date >= start_date)
        if end_date:
            query = query.where(Attendance.attendance_date <= end_date)
