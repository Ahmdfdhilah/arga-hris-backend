...
class LeaveRequestCreateRequest(BaseModel):
    ...
    employee_id: int = Field(
        ..., gt=0, description="ID employee dari workforce service"
    )
    leave_type: LeaveType = Field(
        ..., description=f"Jenis cuti ({LeaveType.values_string()})"
    )
    start_date: date = Field(..., description="Tanggal mulai cuti")
    end_date: date = Field(..., description="Tanggal akhir cuti")
    reason: str = Field(
        ..., min_length=1, max_length=1000, description="Alasan pengajuan cuti"
    )
    replacement_employee_id: Optional[int] = Field(
        None, gt=0, description="ID employee yang menggantikan (opsional)"
    )
    ...
    def validate_end_date(cls, v, info):
        if v and info.data.get("start_date"):
            if v < info.data["start_date"]:
                raise ValueError(
                    "Tanggal akhir cuti harus lebih besar atau sama dengan tanggal mulai cuti"
                )
        return v
...
class LeaveRequestUpdateRequest(BaseModel):
    ...
    leave_type: Optional[LeaveType] = Field(
        None, description=f"Jenis cuti ({LeaveType.values_string()})"
    )
    ...
    reason: Optional[str] = Field(
        None, min_length=1, max_length=1000, description="Alasan pengajuan cuti"
    )
