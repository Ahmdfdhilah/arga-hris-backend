...
from app.grpc.converters import org_unit_to_proto, org_unit_info_to_proto
...
class OrgUnitHandler(org_unit_pb2_grpc.OrgUnitServiceServicer):
    ...
    async def GetOrgUnit(
        ...
        try:
            ...
            async with session:
                ...
                if not org_unit:
                    await context.abort(grpc.StatusCode.NOT_FOUND, f"OrgUnit {request.org_unit_id} tidak ditemukan")
    ...
    async def GetOrgUnitByCode(
        ...
        try:
            ...
            async with session:
                ...
                if not org_unit:
                    await context.abort(grpc.StatusCode.NOT_FOUND, f"OrgUnit dengan kode {request.org_unit_code} tidak ditemukan")
        ...
        try:
            ...
            async with session:
                ...
                org_units, total = await service.list_org_units(
                    ...
                    parent_id=request.org_unit_parent_id if request.HasField("org_unit_parent_id") else None,
                    type_filter=request.org_unit_type if request.HasField("org_unit_type") else None,
                )
        ...
        try:
            ...
            async with session:
                org_unit = await service.create_org_unit(
                    ...
                    parent_id=request.org_unit_parent_id if request.HasField("org_unit_parent_id") else None,
                    head_id=request.org_unit_head_id if request.HasField("org_unit_head_id") else None,
                    description=request.org_unit_description if request.HasField("org_unit_description") else None,
                )
        ...
        try:
            ...
            async with session:
                org_unit = await service.update_org_unit(
                    ...
                    name=request.org_unit_name if request.HasField("org_unit_name") else None,
                    type=request.org_unit_type if request.HasField("org_unit_type") else None,
                    parent_id=request.org_unit_parent_id if request.HasField("org_unit_parent_id") else None,
                    head_id=request.org_unit_head_id if request.HasField("org_unit_head_id") else None,
                    description=request.org_unit_description if request.HasField("org_unit_description") else None,
    ...
    async def DeleteOrgUnit(
        ...
        try:
            service, session = await self._get_service()
            async with session:
                await service.soft_delete_org_unit(request.org_unit_id, request.deleted_by)
                await session.commit()
    ...
    async def GetOrgUnitHierarchy(
        ...
        logger.info(f"gRPC GetOrgUnitHierarchy called")
        
        try:
            service, session = await self._get_service()
            async with session:
                root_id = request.root_org_unit_id if request.HasField("root_org_unit_id") else None
                ...
                def convert_to_hierarchy(node) -> org_unit_pb2.OrgUnitHierarchy:
                    proto = org_unit_pb2.OrgUnitHierarchy(
                        org_unit=org_unit_to_proto(node["unit"]) if "unit" in node else org_unit_to_proto(node),
                    )
                    if "children" in node:
        ...
        try:
            ...
            async with session:
                ...
                if not org_unit:
                    ...
                    context.set_details(f"OrgUnit {request.org_unit_id} tidak ditemukan")
                    return org_unit_pb2.ListOrgUnitsResponse()
