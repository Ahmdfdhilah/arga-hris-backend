...
class EventConsumer:
    ...
    def start_background(
        self, queue_name: str, prefetch_count: int = 10
    ) -> asyncio.Task:
        """
        ...
        """
        self._task = asyncio.create_task(self.start(queue_name, prefetch_count))
        return self._task
    ...
    async def _process_message(self, message: AbstractIncomingMessage) -> None:
        ...
        async with message.process(requeue=False):
            try:
                ...
                if not handlers:
                    logger.warning(
                        f"No handler registered for: {event.get_routing_key()}"
                    )
                    return
    ...
    def _parse_message(self, message: AbstractIncomingMessage) -> Optional[DomainEvent]:
        ...
        try:
            ...
            return DomainEvent(
                ...
                event_action=body.get(
                    "event_type", ""
                ),  # Publisher sends action as "event_type"
                entity_id=body.get("entity_id"),
                data=body.get("data", {}),
                timestamp=timestamp,
                source_service=body.get(
