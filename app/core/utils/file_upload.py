...
async def validate_file_type(file: UploadFile, allowed_types: set) -> str:
    ...
    if content_type not in allowed_types:
        logger.warning(
            f"File type '{content_type}' not allowed for file: {file.filename}"
        )
    ...
    if kind is not None:
        ...
        if detected_mime not in allowed_types:
            logger.warning(
                f"File content mismatch for {file.filename}: detected {detected_mime}, expected one of {allowed_types}"
            )
            raise FileValidationError(
    ...
    logger.debug(
        f"Magic bytes detection failed, using content_type: {content_type} for {file.filename}"
    )
    return content_type
...
async def validate_file_size(file: UploadFile, max_size: int) -> int:
    ...
    if file_size > max_size:
        ...
        logger.warning(
            f"File {file.filename} too large: {file_size_mb:.2f} MB (max: {max_size_mb:.2f} MB)"
        )
        raise FileValidationError(
...
async def validate_image_file(
    ...
    logger.info(
        f"Image file validated: {file.filename}, type: {mime_type}, size: {file_size} bytes"
    )
...
async def upload_file_to_gcp(
    ...
    """
    logger.info(
        f"Uploading file to GCP: {file.filename} for {entity_type}/{entity_id}/{subfolder}"
    )
